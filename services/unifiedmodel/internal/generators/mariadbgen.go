...
// MariaDBGenerator implements SQL generation for MariaDB
...
// Override BaseGenerator methods to provide MariaDB-specific implementations

// Structural organization
func (g *MariaDBGenerator) GenerateCreateDatabaseSQL(database unifiedmodel.Database) (string, error) {
	if database.Name == "" {
...
func (g *MariaDBGenerator) GenerateCreateTableSQL(table unifiedmodel.Table) (string, error) {
	if table.Name == "" {
	...
	// Add table options (MariaDB-specific)
	...
	// Add ROW_FORMAT (MariaDB supports various row formats)
	if rowFormat, ok := table.Options["row_format"].(string); ok && rowFormat != "" {
		sb.WriteString(fmt.Sprintf(" ROW_FORMAT=%s", rowFormat))
	}
...
func (g *MariaDBGenerator) GenerateCreateViewSQL(view unifiedmodel.View) (string, error) {
	if view.Name == "" {
...
func (g *MariaDBGenerator) GenerateCreateIndexSQL(index unifiedmodel.Index) (string, error) {
	if index.Name == "" {
	...
	// Determine table name - this should be provided in context
	...
	// Add index method if specified (MariaDB supports BTREE, HASH, etc.)
	if index.Type != "" {
...
func (g *MariaDBGenerator) GenerateCreateSequenceSQL(seq unifiedmodel.Sequence) (string, error) {
	// MariaDB 10.3+ supports sequences natively
	if seq.Name == "" {
...
func (g *MariaDBGenerator) GenerateCreateFunctionSQL(fn unifiedmodel.Function) (string, error) {
	if fn.Name == "" {
...
func (g *MariaDBGenerator) GenerateCreateProcedureSQL(procedure unifiedmodel.Procedure) (string, error) {
	if procedure.Name == "" {
	...
	for _, arg := range procedure.Arguments {
		paramMode := "IN" // Default mode for MariaDB procedures
...
func (g *MariaDBGenerator) GenerateCreateTriggerSQL(trigger unifiedmodel.Trigger) (string, error) {
	if trigger.Name == "" {
	...
	// Add events (MariaDB supports only one event per trigger)
	...
	// Add trigger body (MariaDB triggers reference procedures)
	sb.WriteString("\nBEGIN\n")
...
func (g *MariaDBGenerator) GenerateCreateUserSQL(user unifiedmodel.DBUser) (string, error) {
	if user.Name == "" {
...
func (g *MariaDBGenerator) GenerateCreateRoleSQL(role unifiedmodel.DBRole) (string, error) {
	if role.Name == "" {
		return "", fmt.Errorf("role name cannot be empty")
	}

	// MariaDB 10.0.5+ supports roles
	var sb strings.Builder
...
func (g *MariaDBGenerator) GenerateCreateGrantSQL(grant unifiedmodel.Grant) (string, error) {
	if grant.Principal == "" {
...
func (g *MariaDBGenerator) GenerateCreateExtensionSQL(extension unifiedmodel.Extension) (string, error) {
	// MariaDB uses plugins similar to MySQL
	if extension.Name == "" {
...
func (g *MariaDBGenerator) GenerateSchema(model *unifiedmodel.UnifiedModel) (string, []string, error) {
	if model == nil {
	...
	// Generate in dependency order for MariaDB
	...
	// 2. Sequences (MariaDB 10.3+ supports native sequences)
	...
	// 5. Indexes (depend on tables/views)
...
func (g *MariaDBGenerator) GenerateCreateStatements(model *unifiedmodel.UnifiedModel) ([]string, error) {
	if model == nil {
		return nil, fmt.Errorf("unified model cannot be nil")
	}
...
func (g *MariaDBGenerator) generateColumnDefinition(col unifiedmodel.Column) (string, error) {
	if col.Name == "" {
...
func (g *MariaDBGenerator) generateConstraintDefinition(constraint unifiedmodel.Constraint) (string, error) {
...
func (g *MariaDBGenerator) mapDataType(dataType string) string {
	// Map common data types to MariaDB equivalents
	...
	case "datetime", "timestamp":
		return "DATETIME"
	case "json":
		return "JSON" // MariaDB 10.2+ supports JSON
	case "binary":
		return "BINARY"
	case "varbinary":
	...
	case "uuid":
		return "CHAR(36)" // MariaDB doesn't have native UUID type
	default:
		// Return as-is for MariaDB-specific types
		return dataType
	}
}
