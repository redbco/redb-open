...
func (s *Service) Initialize(ctx context.Context, cfg *config.Config) error {
	...
	// Set restart keys specific to Unified Model service
	cfg.SetRestartKeys([]string{
		"services.unifiedmodel.grpc_port",
		"services.unifiedmodel.timeout",
...
func (s *Service) GetCapabilities() *supervisorv1.ServiceCapabilities {
	return &supervisorv1.ServiceCapabilities{
		...
		RequiredConfig: map[string]string{
			"services.unifiedmodel.grpc_port": "gRPC port for the Unified Model service",
			"services.unifiedmodel.timeout":   "Request timeout in seconds",
		},
	}
}
