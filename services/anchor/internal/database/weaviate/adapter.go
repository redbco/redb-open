...
func (a *Adapter) Type() dbcapabilities.DatabaseType {
	return dbcapabilities.Weaviate
}

func (a *Adapter) Capabilities() dbcapabilities.Capability {
	return dbcapabilities.MustGet(dbcapabilities.Weaviate)
}
...
func (a *Adapter) Connect(ctx context.Context, config adapter.ConnectionConfig) (adapter.Connection, error) {
	...
	if err != nil {
		return nil, adapter.NewConnectionError(dbcapabilities.Weaviate, config.Host, config.Port, err)
	}

	weaviateClient, ok := client.DB.(*WeaviateClient)
	if !ok {
		return nil, adapter.NewConfigurationError(dbcapabilities.Weaviate, "connection", "invalid weaviate connection type")
	}
...
func (c *Connection) Type() dbcapabilities.DatabaseType { return dbcapabilities.Weaviate }
func (c *Connection) IsConnected() bool                 { return atomic.LoadInt32(&c.connected) == 1 }
func (c *Connection) Ping(ctx context.Context) error {
	// Weaviate uses HTTP client, simple check if client is initialized
	if c.client == nil || c.client.BaseURL == "" {
		return adapter.NewDatabaseError(dbcapabilities.Weaviate, "ping", fmt.Errorf("client not initialized"))
	}
	return nil
}
func (c *Connection) Close() error {
	atomic.StoreInt32(&c.connected, 0)
	return nil // Weaviate is HTTP-based, no connection to close
}
...
func (c *Connection) ReplicationOperations() adapter.ReplicationOperator {
	return adapter.NewUnsupportedReplicationOperator(dbcapabilities.Weaviate)
}
...
func (i *InstanceConnection) Type() dbcapabilities.DatabaseType { return dbcapabilities.Weaviate }
...
func (i *InstanceConnection) Ping(ctx context.Context) error {
	if i.client == nil || i.client.BaseURL == "" {
		return adapter.NewDatabaseError(dbcapabilities.Weaviate, "ping", fmt.Errorf("client not initialized"))
	}
	return nil
}
...
func (i *InstanceConnection) ListDatabases(ctx context.Context) ([]string, error) {
	...
	if err != nil {
		return nil, adapter.WrapError(dbcapabilities.Weaviate, "list_databases", err)
	}
