...
// DataOps implements data operations for Synapse.
type DataOps struct {
	conn *Connection
}

// Fetch retrieves rows from Synapse.
...
func (d *DataOps) FetchWithColumns(ctx context.Context, table string, columns []string, limit int) ([]map[string]interface{}, error) {
	...
	query := fmt.Sprintf("SELECT TOP %d %s FROM [%s]", limit, selectClause, table)
...
// Insert inserts rows into Synapse.
func (d *DataOps) Insert(ctx context.Context, table string, data []map[string]interface{}) (int64, error) {
	...
	for _, row := range data {
		...
		_, err := d.conn.client.DB().ExecContext(ctx, query, values...)
		if err != nil {
...
// Update updates rows in Synapse.
...
// Upsert performs upsert operation in Synapse.
...
// Delete deletes rows from Synapse.
...
func (d *DataOps) Stream(ctx context.Context, params adapter.StreamParams) (adapter.StreamResult, error) {
	...
	query := fmt.Sprintf("SELECT %s FROM [%s] ORDER BY (SELECT NULL) OFFSET %d ROWS FETCH NEXT %d ROWS ONLY",
		selectClause, params.Table, params.Offset, params.BatchSize)
