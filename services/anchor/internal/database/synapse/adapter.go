...
// Adapter implements adapter.DatabaseAdapter for Azure Synapse Analytics.
type Adapter struct{}

// NewAdapter creates a new Synapse adapter instance.
...
// ConnectInstance establishes an instance-level connection to Synapse.
func (a *Adapter) ConnectInstance(ctx context.Context, config adapter.InstanceConfig) (adapter.InstanceConnection, error) {
	client, err := NewSynapseClientFromInstance(ctx, config)
	if err != nil {
		return nil, adapter.NewConnectionError(
			dbcapabilities.Synapse,
			config.Host,
...
// Connection implements adapter.Connection for Synapse.
...
// Raw returns the underlying Synapse client.
...
// InstanceConnection implements adapter.InstanceConnection for Synapse.
