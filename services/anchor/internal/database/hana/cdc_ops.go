...
// ParseEvent converts a HANA-specific raw event to a standardized CDCEvent.
func (r *ReplicationOps) ParseEvent(ctx context.Context, rawEvent map[string]interface{}) (*adapter.CDCEvent, error) {
	...
	} else if op, ok := rawEvent["cdc_operation"].(string); ok {
		event.Operation = adapter.CDCOperation(strings.ToUpper(op))
	} else {
	...
	} else if schema, ok := rawEvent["schema"].(string); ok {
		event.SchemaName = schema
	}
	...
	if commitID, ok := rawEvent["commit_id"]; ok {
		event.Metadata["commit_id"] = commitID
		event.LSN = fmt.Sprintf("%v", commitID)
	} else if commitID, ok := rawEvent["cdc_commit_id"]; ok {
		event.Metadata["commit_id"] = commitID
		event.LSN = fmt.Sprintf("%v", commitID)
	}
	...
	} else if timestamp, ok := rawEvent["cdc_timestamp"].(time.Time); ok {
		event.Timestamp = timestamp
	}

	// Validate the event
	if err := event.Validate(); err != nil {
		return nil, adapter.WrapError(dbcapabilities.HANA, "parse_cdc_event", err)
	}

	return event, nil
}

// ApplyCDCEvent applies a standardized CDC event to SAP HANA.
func (r *ReplicationOps) ApplyCDCEvent(ctx context.Context, event *adapter.CDCEvent) error {
	// Validate event
	if err := event.Validate(); err != nil {
		return adapter.WrapError(dbcapabilities.HANA, "apply_cdc_event", err)
	}
...
// applyCDCInsert handles INSERT operations for SAP HANA.
func (r *ReplicationOps) applyCDCInsert(ctx context.Context, event *adapter.CDCEvent) error {
	...
	if err != nil {
		return adapter.WrapError(dbcapabilities.HANA, "apply_cdc_insert", err)
	}

	return nil
}

// applyCDCUpdate handles UPDATE operations for SAP HANA.
func (r *ReplicationOps) applyCDCUpdate(ctx context.Context, event *adapter.CDCEvent) error {
	...
	_, err := r.conn.db.ExecContext(ctx, query, values...)
	if err != nil {
		return adapter.WrapError(dbcapabilities.HANA, "apply_cdc_update", err)
	}

	return nil
}

// applyCDCDelete handles DELETE operations for SAP HANA.
func (r *ReplicationOps) applyCDCDelete(ctx context.Context, event *adapter.CDCEvent) error {
	...
	_, err := r.conn.db.ExecContext(ctx, query, values...)
	if err != nil {
		return adapter.WrapError(dbcapabilities.HANA, "apply_cdc_delete", err)
	}

	return nil
}

// applyCDCTruncate handles TRUNCATE operations for SAP HANA.
func (r *ReplicationOps) applyCDCTruncate(ctx context.Context, event *adapter.CDCEvent) error {
	...
	if err != nil {
		return adapter.WrapError(dbcapabilities.HANA, "apply_cdc_truncate", err)
	}
