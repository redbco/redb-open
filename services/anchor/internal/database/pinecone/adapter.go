...
func (a *Adapter) Type() dbcapabilities.DatabaseType {
	return dbcapabilities.Pinecone
}

func (a *Adapter) Capabilities() dbcapabilities.Capability {
	return dbcapabilities.MustGet(dbcapabilities.Pinecone)
}
...
func (a *Adapter) Connect(ctx context.Context, config adapter.ConnectionConfig) (adapter.Connection, error) {
	...
	if err != nil {
		return nil, adapter.NewConnectionError(dbcapabilities.Pinecone, config.Host, config.Port, err)
	}

	pineconeClient, ok := client.DB.(*PineconeClient)
	if !ok {
		return nil, adapter.NewConfigurationError(dbcapabilities.Pinecone, "connection", "invalid pinecone connection type")
	}
...
func (c *Connection) Type() dbcapabilities.DatabaseType { return dbcapabilities.Pinecone }
func (c *Connection) IsConnected() bool                 { return atomic.LoadInt32(&c.connected) == 1 }
func (c *Connection) Ping(ctx context.Context) error {
	// Pinecone uses HTTP client, simple check if client is initialized
	if c.client == nil || c.client.APIKey == "" {
		return adapter.NewDatabaseError(dbcapabilities.Pinecone, "ping", fmt.Errorf("client not initialized"))
	}
	return nil
}
func (c *Connection) Close() error {
	atomic.StoreInt32(&c.connected, 0)
	return nil // Pinecone is HTTP-based, no connection to close
}
...
func (c *Connection) ReplicationOperations() adapter.ReplicationOperator {
	return adapter.NewUnsupportedReplicationOperator(dbcapabilities.Pinecone)
}
...
func (i *InstanceConnection) Type() dbcapabilities.DatabaseType { return dbcapabilities.Pinecone }
...
func (i *InstanceConnection) Ping(ctx context.Context) error {
	if i.client == nil || i.client.APIKey == "" {
		return adapter.NewDatabaseError(dbcapabilities.Pinecone, "ping", fmt.Errorf("client not initialized"))
	}
	return nil
}
...
func (i *InstanceConnection) ListDatabases(ctx context.Context) ([]string, error) {
	...
	if err != nil {
		return nil, adapter.WrapError(dbcapabilities.Pinecone, "list_databases", err)
	}
