...
func (a *Adapter) Type() dbcapabilities.DatabaseType {
	return dbcapabilities.Chroma
}

func (a *Adapter) Capabilities() dbcapabilities.Capability {
	return dbcapabilities.MustGet(dbcapabilities.Chroma)
}
...
func (a *Adapter) Connect(ctx context.Context, config adapter.ConnectionConfig) (adapter.Connection, error) {
	...
	if err != nil {
		return nil, adapter.NewConnectionError(dbcapabilities.Chroma, config.Host, config.Port, err)
	}

	chromaClient, ok := client.DB.(*ChromaClient)
	if !ok {
		return nil, adapter.NewConfigurationError(dbcapabilities.Chroma, "connection", "invalid chroma connection type")
	}
...
func (c *Connection) Type() dbcapabilities.DatabaseType { return dbcapabilities.Chroma }
func (c *Connection) IsConnected() bool                 { return atomic.LoadInt32(&c.connected) == 1 }
func (c *Connection) Ping(ctx context.Context) error {
	// Chroma uses HTTP client, simple check if client is initialized
	if c.client == nil || c.client.BaseURL == "" {
		return adapter.NewDatabaseError(dbcapabilities.Chroma, "ping", fmt.Errorf("client not initialized"))
	}
	return nil
}
func (c *Connection) Close() error {
	atomic.StoreInt32(&c.connected, 0)
	return nil // Chroma is HTTP-based, no connection to close
}
...
func (c *Connection) ReplicationOperations() adapter.ReplicationOperator {
	return adapter.NewUnsupportedReplicationOperator(dbcapabilities.Chroma)
}
...
func (i *InstanceConnection) Type() dbcapabilities.DatabaseType { return dbcapabilities.Chroma }
...
func (i *InstanceConnection) ListDatabases(ctx context.Context) ([]string, error) {
	...
	if err != nil {
		return nil, adapter.WrapError(dbcapabilities.Chroma, "list_databases", err)
	}
