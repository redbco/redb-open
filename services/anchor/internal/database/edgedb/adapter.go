...
func (a *Adapter) Type() dbcapabilities.DatabaseType {
	return dbcapabilities.EdgeDB
}

func (a *Adapter) Capabilities() dbcapabilities.Capability {
	return dbcapabilities.MustGet(dbcapabilities.EdgeDB)
}
...
func (a *Adapter) Connect(ctx context.Context, config adapter.ConnectionConfig) (adapter.Connection, error) {
	...
	if err != nil {
		return nil, adapter.NewConnectionError(dbcapabilities.EdgeDB, config.Host, config.Port, err)
	}

	gelClient, ok := client.DB.(*gel.Client)
	if !ok {
		return nil, adapter.NewConfigurationError(dbcapabilities.EdgeDB, "connection", "invalid edgedb connection type")
	}
...
func (c *Connection) Type() dbcapabilities.DatabaseType { return dbcapabilities.EdgeDB }
func (c *Connection) IsConnected() bool                 { return atomic.LoadInt32(&c.connected) == 1 }
func (c *Connection) Ping(ctx context.Context) error {
	return c.client.Execute(ctx, "SELECT 1;")
}
...
func (c *Connection) ReplicationOperations() adapter.ReplicationOperator {
	return adapter.NewUnsupportedReplicationOperator(dbcapabilities.EdgeDB)
}
...
func (i *InstanceConnection) Type() dbcapabilities.DatabaseType { return dbcapabilities.EdgeDB }
...
func (i *InstanceConnection) Ping(ctx context.Context) error {
	return i.client.Execute(ctx, "SELECT 1;")
}
...
func (i *InstanceConnection) ListDatabases(ctx context.Context) ([]string, error) {
	return []string{}, adapter.NewUnsupportedOperationError(dbcapabilities.EdgeDB, "list databases", "not yet implemented")
}
