...
func ExecuteQuery(db interface{}, query string, args ...interface{}) ([]interface{}, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(valuePtrs...); err != nil {
			return nil, fmt.Errorf("failed to scan postgres result: %w", err)
		}
...
func ExecuteCountQuery(db interface{}, query string) (int64, error) {
	...
	if err := row.Scan(&count); err != nil {
		return 0, fmt.Errorf("failed to scan postgres count result: %w", err)
	}
...
func StreamTableData(db interface{}, tableName string, batchSize int32, offset int64, columns []string) ([]map[string]interface{}, bool, string, error) {
	pool, ok := db.(*pgxpool.Pool)
	if !ok {
		return nil, false, "", fmt.Errorf("invalid postgres connection type")
	}
	...
	if err != nil {
		return nil, false, "", fmt.Errorf("failed to execute postgres streaming query: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(valuePtrs...); err != nil {
			return nil, false, "", fmt.Errorf("failed to scan postgres result: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, false, "", fmt.Errorf("postgres rows iteration error: %w", err)
	}
...
func GetTableRowCount(db interface{}, tableName string, whereClause string) (int64, bool, error) {
	pool, ok := db.(*pgxpool.Pool)
	if !ok {
		return 0, false, fmt.Errorf("invalid postgres connection type")
	}
	...
	row := pool.QueryRow(ctx, query)
	if err := row.Scan(&count); err != nil {
		return 0, false, fmt.Errorf("failed to scan postgres count result: %w", err)
	}

	// PostgreSQL COUNT(*) is always exact, not an estimate
