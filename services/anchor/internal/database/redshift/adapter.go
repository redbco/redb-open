...
// Adapter implements adapter.DatabaseAdapter for Amazon Redshift.
type Adapter struct{}

// NewAdapter creates a new Redshift adapter instance.
...
func (a *Adapter) Connect(ctx context.Context, config adapter.ConnectionConfig) (adapter.Connection, error) {
	// Create Redshift client (uses PostgreSQL driver)
	client, err := NewRedshiftClient(ctx, config)
	if err != nil {
		return nil, adapter.NewConnectionError(
			dbcapabilities.Redshift,
			config.Host,
			config.Port,
			err,
		)
...
// ConnectInstance establishes an instance-level connection to Redshift.
func (a *Adapter) ConnectInstance(ctx context.Context, config adapter.InstanceConfig) (adapter.InstanceConnection, error) {
	client, err := NewRedshiftClientFromInstance(ctx, config)
	if err != nil {
		return nil, adapter.NewConnectionError(
			dbcapabilities.Redshift,
			config.Host,
...
// Connection implements adapter.Connection for Redshift.
...
// Raw returns the underlying Redshift client.
...
// InstanceConnection implements adapter.InstanceConnection for Redshift.
...
// ListDatabases lists all Redshift databases.
...
// CreateDatabase creates a new Redshift database.
...
// DropDatabase deletes a Redshift database.
