...
// ParseEvent converts a DB2-specific raw event to a standardized CDCEvent.
func (r *ReplicationOps) ParseEvent(ctx context.Context, rawEvent map[string]interface{}) (*adapter.CDCEvent, error) {
	...
	if changeID, ok := rawEvent["change_id"]; ok {
		event.Metadata["change_id"] = changeID
		event.LSN = fmt.Sprintf("%v", changeID) // Use change ID as LSN
	}
	...
	if err := event.Validate(); err != nil {
		return nil, adapter.WrapError(dbcapabilities.DB2, "parse_cdc_event", err)
	}

	return event, nil
}

// ApplyCDCEvent applies a standardized CDC event to DB2.
...
func (r *ReplicationOps) ApplyCDCEvent(ctx context.Context, event *adapter.CDCEvent) error {
	// Validate event
	if err := event.Validate(); err != nil {
		return adapter.WrapError(dbcapabilities.DB2, "apply_cdc_event", err)
	}
...
// applyCDCInsert handles INSERT operations for DB2.
func (r *ReplicationOps) applyCDCInsert(ctx context.Context, event *adapter.CDCEvent) error {
	...
	for col, val := range event.Data {
		...
		placeholders = append(placeholders, "?") // DB2 uses ? placeholders
	...
	if err != nil {
		return adapter.WrapError(dbcapabilities.DB2, "apply_cdc_insert", err)
	}

	return nil
}

// applyCDCUpdate handles UPDATE operations for DB2.
func (r *ReplicationOps) applyCDCUpdate(ctx context.Context, event *adapter.CDCEvent) error {
	...
	if len(setClauses) == 0 {
		return nil // No columns to update
	}

	// Build WHERE clause using old data (if available) or current data
	...
	if err != nil {
		return adapter.WrapError(dbcapabilities.DB2, "apply_cdc_update", err)
	}

	// Check if any rows were affected
	rowsAffected, _ := result.RowsAffected()
	if rowsAffected == 0 {
		// Log warning but don't fail - row might not exist
		return nil
	}
...
func (r *ReplicationOps) applyCDCDelete(ctx context.Context, event *adapter.CDCEvent) error {
	...
	_, err := r.conn.db.ExecContext(ctx, query, values...)
	if err != nil {
		return adapter.WrapError(dbcapabilities.DB2, "apply_cdc_delete", err)
	}

	return nil
}

// applyCDCTruncate handles TRUNCATE operations for DB2.
func (r *ReplicationOps) applyCDCTruncate(ctx context.Context, event *adapter.CDCEvent) error {
	query := fmt.Sprintf("TRUNCATE TABLE %s IMMEDIATE", QuoteIdentifier(event.TableName))

	_, err := r.conn.db.ExecContext(ctx, query)
	if err != nil {
		return adapter.WrapError(dbcapabilities.DB2, "apply_cdc_truncate", err)
	}
...
// isMetadataField checks if a field name is a metadata field that should not be inserted.
