...
func (a *Adapter) Type() dbcapabilities.DatabaseType {
	return dbcapabilities.Milvus
}

func (a *Adapter) Capabilities() dbcapabilities.Capability {
	return dbcapabilities.MustGet(dbcapabilities.Milvus)
}
...
func (a *Adapter) Connect(ctx context.Context, config adapter.ConnectionConfig) (adapter.Connection, error) {
	...
	if err != nil {
		return nil, adapter.NewConnectionError(dbcapabilities.Milvus, config.Host, config.Port, err)
	}

	milvusClient, ok := client.DB.(*MilvusClient)
	if !ok {
		return nil, adapter.NewConfigurationError(dbcapabilities.Milvus, "connection", "invalid milvus connection type")
	}
...
func (c *Connection) Type() dbcapabilities.DatabaseType { return dbcapabilities.Milvus }
func (c *Connection) IsConnected() bool                 { return atomic.LoadInt32(&c.connected) == 1 }
func (c *Connection) Ping(ctx context.Context) error {
	// Milvus uses HTTP client, simple check if client is initialized
	if c.client == nil || c.client.BaseURL == "" {
		return adapter.NewDatabaseError(dbcapabilities.Milvus, "ping", fmt.Errorf("client not initialized"))
	}
	return nil
}
func (c *Connection) Close() error {
	atomic.StoreInt32(&c.connected, 0)
	return nil // Milvus is HTTP-based, no connection to close
}
...
func (c *Connection) ReplicationOperations() adapter.ReplicationOperator {
	return adapter.NewUnsupportedReplicationOperator(dbcapabilities.Milvus)
}
...
func (i *InstanceConnection) Type() dbcapabilities.DatabaseType { return dbcapabilities.Milvus }
...
func (i *InstanceConnection) ListDatabases(ctx context.Context) ([]string, error) {
	...
	if err != nil {
		return nil, adapter.WrapError(dbcapabilities.Milvus, "list_databases", err)
	}
