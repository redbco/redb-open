...
func (a *Adapter) Type() dbcapabilities.DatabaseType {
	return dbcapabilities.Elasticsearch
}

func (a *Adapter) Capabilities() dbcapabilities.Capability {
	return dbcapabilities.MustGet(dbcapabilities.Elasticsearch)
}
...
func (a *Adapter) Connect(ctx context.Context, config adapter.ConnectionConfig) (adapter.Connection, error) {
	...
	if err != nil {
		return nil, adapter.NewConnectionError(dbcapabilities.Elasticsearch, config.Host, config.Port, err)
	}

	esClient, ok := client.DB.(*ElasticsearchClient)
	if !ok {
		return nil, adapter.NewConfigurationError(dbcapabilities.Elasticsearch, "connection", "invalid elasticsearch connection type")
	}
...
func (c *Connection) Type() dbcapabilities.DatabaseType { return dbcapabilities.Elasticsearch }
func (c *Connection) IsConnected() bool                 { return atomic.LoadInt32(&c.connected) == 1 }
func (c *Connection) Ping(ctx context.Context) error {
	_, err := c.client.Client.Ping()
	return err
}
func (c *Connection) Close() error {
	atomic.StoreInt32(&c.connected, 0)
	return nil // Elasticsearch client doesn't need explicit close
}
...
func (c *Connection) ReplicationOperations() adapter.ReplicationOperator {
	return adapter.NewUnsupportedReplicationOperator(dbcapabilities.Elasticsearch)
}
...
func (i *InstanceConnection) Type() dbcapabilities.DatabaseType { return dbcapabilities.Elasticsearch }
...
func (i *InstanceConnection) ListDatabases(ctx context.Context) ([]string, error) {
	...
	if err != nil {
		return nil, adapter.WrapError(dbcapabilities.Elasticsearch, "list_databases", err)
	}
