...
func (a *Adapter) Type() dbcapabilities.DatabaseType {
	return dbcapabilities.ClickHouse
}

func (a *Adapter) Capabilities() dbcapabilities.Capability {
	return dbcapabilities.MustGet(dbcapabilities.ClickHouse)
}
...
func (a *Adapter) Connect(ctx context.Context, config adapter.ConnectionConfig) (adapter.Connection, error) {
	...
	if err != nil {
		return nil, adapter.NewConnectionError(dbcapabilities.ClickHouse, config.Host, config.Port, err)
	}

	conn, ok := client.DB.(ClickhouseConn)
	if !ok {
		return nil, adapter.NewConfigurationError(dbcapabilities.ClickHouse, "connection", "invalid clickhouse connection type")
	}

	return &Connection{
		id:        config.DatabaseID,
		conn:      conn, // Already ClickhouseConn interface
		config:    config,
		adapter:   a,
		connected: 1,
	}, nil
...
func (c *Connection) Type() dbcapabilities.DatabaseType { return dbcapabilities.ClickHouse }
func (c *Connection) IsConnected() bool                 { return atomic.LoadInt32(&c.connected) == 1 }
func (c *Connection) Ping(ctx context.Context) error {
	// ClickHouse Ping via simple query
	return c.conn.Exec(ctx, "SELECT 1")
}
...
func (i *InstanceConnection) Type() dbcapabilities.DatabaseType { return dbcapabilities.ClickHouse }
...
func (i *InstanceConnection) Ping(ctx context.Context) error {
	return i.conn.Exec(ctx, "SELECT 1")
}
...
func (i *InstanceConnection) Raw() interface{}                 { return i.conn }
...
func (i *InstanceConnection) ListDatabases(ctx context.Context) ([]string, error) {
	rows, err := i.conn.Query(ctx, "SHOW DATABASES")
	if err != nil {
		return nil, adapter.WrapError(dbcapabilities.ClickHouse, "list_databases", err)
	}
	defer rows.Close()
...
func (i *InstanceConnection) CreateDatabase(ctx context.Context, name string, options map[string]interface{}) error {
	return CreateDatabase(ctx, i.conn, name, options) // Legacy function expects interface{}
}

func (i *InstanceConnection) DropDatabase(ctx context.Context, name string, options map[string]interface{}) error {
	return DropDatabase(ctx, i.conn, name, options) // Legacy function expects interface{}
}
