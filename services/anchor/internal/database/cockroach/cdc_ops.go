...
// ParseEvent converts a CockroachDB changefeed event to a standardized CDCEvent.
func (r *ReplicationOps) ParseEvent(ctx context.Context, rawEvent map[string]interface{}) (*adapter.CDCEvent, error) {
	...
	// Extract table name
	if table, ok := rawEvent["table"].(string); ok {
		...
		).WithContext("error", "missing table field")
	}

	// CockroachDB changefeeds use "after" for the new state and "before" for the old state
	var hasAfter, hasBefore bool
	if after, ok := rawEvent["after"].(map[string]interface{}); ok && after != nil {
		event.Data = after
		hasAfter = true
	}
	if before, ok := rawEvent["before"].(map[string]interface{}); ok && before != nil {
		event.OldData = before
		hasBefore = true
	}

	// Determine operation type based on presence of before/after
	if hasAfter && !hasBefore {
		...
		// For DELETE, move old data to data if data is empty
		if len(event.Data) == 0 {
		...
		return nil, adapter.NewDatabaseError(
			...
			"parse_cdc_event",
			adapter.ErrInvalidData,
		).WithContext("error", "cannot determine operation type from changefeed event")
	}

	// Extract metadata
	if updated, ok := rawEvent["updated"].(string); ok {
		event.Metadata["updated"] = updated
		event.LSN = updated // Use updated timestamp as LSN
		// Try to parse the timestamp
		if t, err := time.Parse(time.RFC3339Nano, updated); err == nil {
			event.Timestamp = t
		}
	}
	if key, ok := rawEvent["key"].(map[string]interface{}); ok {
		event.Metadata["key"] = key
	}

	// Validate the event
	if err := event.Validate(); err != nil {
		return nil, adapter.WrapError(dbcapabilities.CockroachDB, "parse_cdc_event", err)
	}

	return event, nil
}

// ApplyCDCEvent applies a standardized CDC event to CockroachDB.
func (r *ReplicationOps) ApplyCDCEvent(ctx context.Context, event *adapter.CDCEvent) error {
	// Validate event
	if err := event.Validate(); err != nil {
		return adapter.WrapError(dbcapabilities.CockroachDB, "apply_cdc_event", err)
	}
...
// applyCDCInsert handles INSERT operations for CockroachDB.
func (r *ReplicationOps) applyCDCInsert(ctx context.Context, event *adapter.CDCEvent) error {
	...
	if err != nil {
		return adapter.WrapError(dbcapabilities.CockroachDB, "apply_cdc_insert", err)
	}

	return nil
}

// applyCDCUpdate handles UPDATE operations for CockroachDB.
func (r *ReplicationOps) applyCDCUpdate(ctx context.Context, event *adapter.CDCEvent) error {
	...
	if len(whereClauses) == 0 {
		whereClauses = []string{"true"} // Fallback if no WHERE conditions
	}
	...
	if err != nil {
		return adapter.WrapError(dbcapabilities.CockroachDB, "apply_cdc_update", err)
	}
...
// applyCDCDelete handles DELETE operations for CockroachDB.
func (r *ReplicationOps) applyCDCDelete(ctx context.Context, event *adapter.CDCEvent) error {
	...
	result, err := r.conn.pool.Exec(ctx, query, values...)
	if err != nil {
		return adapter.WrapError(dbcapabilities.CockroachDB, "apply_cdc_delete", err)
	}
...
// applyCDCTruncate handles TRUNCATE operations for CockroachDB.
func (r *ReplicationOps) applyCDCTruncate(ctx context.Context, event *adapter.CDCEvent) error {
	...
	if err != nil {
		return adapter.WrapError(dbcapabilities.CockroachDB, "apply_cdc_truncate", err)
	}
...
// quoteIdentifier quotes a CockroachDB identifier (table or column name).
func (r *ReplicationOps) quoteIdentifier(identifier string) string {
	// CockroachDB uses double quotes for identifiers (PostgreSQL-compatible)
	// Escape any double quotes in the identifier
	escaped := strings.ReplaceAll(identifier, `"`, `""`)
	return fmt.Sprintf(`"%s"`, escaped)
}

// quoteIdentifiers quotes multiple CockroachDB identifiers.
