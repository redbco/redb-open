...
// SchemaOps implements adapter.SchemaOperator for CockroachDB.
type SchemaOps struct {
	conn *Connection
}

// DiscoverSchema retrieves the complete schema of the CockroachDB database.
func (s *SchemaOps) DiscoverSchema(ctx context.Context) (*unifiedmodel.UnifiedModel, error) {
	...
	if err != nil {
		return nil, adapter.WrapError(dbcapabilities.CockroachDB, "discover_schema", err)
	}
	return um, nil
}
...
func (s *SchemaOps) CreateStructure(ctx context.Context, model *unifiedmodel.UnifiedModel) error {
	...
	if err != nil {
		return adapter.WrapError(dbcapabilities.CockroachDB, "create_structure", err)
	}
	return nil
}
...
func (s *SchemaOps) ListTables(ctx context.Context) ([]string, error) {
	...
	if err != nil {
		return nil, adapter.WrapError(dbcapabilities.CockroachDB, "list_tables", err)
	}
	defer rows.Close()
