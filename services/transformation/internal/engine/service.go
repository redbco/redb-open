...
func (s *Service) Initialize(ctx context.Context, cfg *config.Config) error {
	...
	// Set restart keys specific to Transformation service
	cfg.SetRestartKeys([]string{
		"services.transformation.grpc_port",
		"services.transformation.timeout",
...
func (s *Service) GetCapabilities() *supervisorv1.ServiceCapabilities {
	return &supervisorv1.ServiceCapabilities{
		...
		RequiredConfig: map[string]string{
			"services.transformation.grpc_port": "gRPC port for the Transformation service",
			"services.transformation.timeout":   "Request timeout in seconds",
		},
	}
}
