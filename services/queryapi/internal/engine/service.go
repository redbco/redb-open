...
func (s *Service) SetGRPCServer(server *grpc.Server) {
	s.grpcServer = server
	// Query API doesn't need gRPC server registration (it's HTTP-only)
...
func (s *Service) Initialize(ctx context.Context, cfg *config.Config) error {
	...
	// Set restart keys specific to Query API service
	cfg.SetRestartKeys([]string{
		"services.queryapi.http_port", // Fixed: Use hyphenated name
		"services.queryapi.timeout",   // Fixed: Use hyphenated name
		"services.core.grpc_address",  // Added: Core service address
		// Add other configuration keys that require service restart
	})

	// Initialize the Query API engine
...
func (s *Service) GetCapabilities() *supervisorv1.ServiceCapabilities {
	return &supervisorv1.ServiceCapabilities{
		...
		Dependencies: []string{
			"supervisor",
			"core", // Added: Query API depends on core service
		},
		RequiredConfig: map[string]string{
			"services.queryapi.http_port": "HTTP port for the Query API service (fallback)",
			"services.queryapi.timeout":   "Request timeout in seconds",
